# TriDAS ERSAP Integration Example

[![Build Status](https://travis-ci.org/JeffersonLab/ersap-tridas.svg?branch=master)](https://travis-ci.org/JeffersonLab/ersap-tridas)

Example of a TriDAS application presenting as a CLARA service.
...
Here will go all supporting TriDAS classes necessary for stream processing.
...
So, if TriDAS team decides to use it's own data type then here will go code 
...
The `service`  CmakeList.txt creates the `libTriDASService.so` (or dylib) artifact
...
There is a `tridas.clara` script ready in the root of the repository:

      > $CLARA_HOME/bin/clara-shell tridas.clara
